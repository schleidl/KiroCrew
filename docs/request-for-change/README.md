...
| [rfc-agentcore-remote-agents.md](rfc-agentcore-remote-agents.md) | `draft` | Nothing. Verified at `c719ef81a`: no code invokes AgentCore, `agent_sdk/backends.py` has no remote backend id, and the ACP transport is a subprocess pipe (`acp/client.py` writes `self._process.stdin` directly) with no stream-pair seam. What the RFC builds on does exist — the argv-only harness seam (`acp/harness/base.py`), `capabilities.agentcore` and the `agentcore-inbound` fence, and the peer-relay precedent for rendering a remote turn through local frames (`cloud/remote_relay.py`). Plan: [`plans/2026-09-14-agentcore-remote-agents.md`](plans/2026-09-14-agentcore-remote-agents.md) |
