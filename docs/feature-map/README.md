...
| Subagents | Background agent runs spawned from a session. A run may execute REMOTELY — `executor=agentcore` puts it in a Bedrock AgentCore microVM in the operator's own AWS account — and such a run carries a `remoteExecutor` badge on its card, present only when the run is not local | Chat activity viewer; rail badge | `pages/chat/ActivityViewer.tsx` | `handlers/messaging.py` | `GET,POST /api/spawn`, `POST /api/spawn/stop-all`, `GET /api/spawn/{agent_id}`, `POST /api/spawn/{agent_id}/steer` |
