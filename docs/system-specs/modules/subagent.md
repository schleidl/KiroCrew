...
- `executor` (str, optional): WHERE the subagent runs. `""` (the default) is this
  machine, in the gateway's own sandbox; `"agentcore"` is a `kiro-cli acp` child inside
  a Bedrock AgentCore microVM in the operator's own AWS account, reached through the
  stdio shim and the bridge (`docs/system-specs/modules/harness-parity.md`, §"The
  AgentCore remote harness"). Batch-wide like `model` and `reasoning_effort`, because a
  wave is one delegation decision and a per-task executor would run half a fan-out
  locally. Validated STRICTLY at `/api/spawn`, which answers 400 naming the accepted set
  rather than letting an unrecognised value degrade at the selection gate: a caller that
  asked for a remote executor and silently got a local run has been answered with the
  opposite of its request. A recognised value is then subject to
  `capabilities.remote_exec`, which must be AFFIRMATIVELY granted — an omitted
  capability refuses the spawn and names the scope, and the refusal is never retried
  locally. Any non-empty value FORCES the dedicated-process arm (`subagent_manager/run.py`);
  the shared runtime is a process of the parent's own harness, so a shared-arm remote
  spawn would come up on the wrong harness with nothing red to say so. Carried through
  the stagger queue by three parts that are one mechanism — the keyword-only parameter on
  the `SubagentManager.spawn` facade, the `"executor"` entry in the queued params dict,
  and the drain's `spawn(**params)` — because the drain reads that dict and nothing else,
  so an executor missing from it drains as a local run. NOT accepted by
  `spawn_continue`: a remote session is single-turn, so a finished one cannot be resumed
  in place and a follow-up needs a new spawn seeded with a summary.
